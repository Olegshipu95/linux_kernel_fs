...
static u32
inode_to_index (struct inode *inode)
{
  return inode->i_ino - 2;
}

static int
simplefs_read_sector (struct super_block *sb, u32 index, u32 sec_off,
...
static int
simplefs_write_sector (struct super_block *sb, u32 index, u32 sec_off,
                       const void *buf, unsigned len, unsigned offset_in_sec)
{
  struct simplefs_sb_info *sbi = sb->s_fs_info;
  struct buffer_head *bh;
  sector_t sec;
...
static ssize_t
simplefs_read_file (struct file *file, char __user *buf, size_t len,
                    loff_t *ppos)
{
  ...
  while (done < len)
    {
      ...
      chunk = min_t (unsigned, len - done, sbi->sector_size - off);
...
static ssize_t
simplefs_write_file (struct file *file, const char __user *buf, size_t len,
                     loff_t *ppos)
{
  struct inode *inode = file_inode (file);
  struct super_block *sb = inode->i_sb;
  ...
  meta.data_crc
      = cpu_to_le32 (simplefs_data_crc (sb, index, sbi->max_filename_len));
