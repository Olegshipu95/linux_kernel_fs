...
static void
update_cached_inode_size (struct super_block *sb, u32 index, loff_t size)
{
  struct inode *inode;
...
static int
find_file_index (struct super_block *sb, const char *name, u32 *out)
{
...
static int
ioctl_zero_files (struct super_block *sb)
{
  struct simplefs_sb_info *sbi = sb->s_fs_info;
  u32 i;
      ...
      for (j = 0; j < meta.sectors_used; j++)
        {
          ...
          if (j == 0)
            memset (bh->b_data + off_in_block + SIMPLEFS_META_SIZE, 0,
                    sbi->sector_size - SIMPLEFS_META_SIZE);
          else
            memset (bh->b_data + off_in_block, 0, sbi->sector_size);
...
static int
ioctl_get_meta (struct file *file, unsigned long arg)
{
...
static int
ioctl_get_sector_map (struct file *file, unsigned long arg)
{
  struct inode *inode = file_inode (file);
  struct super_block *sb = inode->i_sb;
  struct simplefs_sb_info *sbi = sb->s_fs_info;
...

  {
    ...
    for (i = 0; i < meta.sectors_used && i < SIMPLEFS_MAX_FILE_SECTORS; i++)
...
long
simplefs_ioctl (struct file *file, unsigned int cmd, unsigned long arg)
{
  struct inode *inode = file_inode (file);
  struct super_block *sb = inode->i_sb;
