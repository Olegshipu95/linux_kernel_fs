...
simplefs_sector_to_block (sector_t sector)
{
  return sector / (SIMPLEFS_BLOCK_SIZE / SIMPLEFS_SECTOR_SIZE);
}

static inline unsigned int
simplefs_sector_block_offset (sector_t sector)
{
  return (sector % (SIMPLEFS_BLOCK_SIZE / SIMPLEFS_SECTOR_SIZE))
...
simplefs_bread_bdev_sector (struct block_device *bdev, sector_t sector)
{
...
static inline struct buffer_head *
simplefs_bread_sb_sector (struct super_block *sb, sector_t sector)
{
...
static inline sector_t
simplefs_file_start (const struct simplefs_sb_info *sbi, u32 index)
{
  return sbi->data_start + (sector_t)index * sbi->max_file_sectors;
}

static inline u64
simplefs_file_data_size (u32 sectors)
{
