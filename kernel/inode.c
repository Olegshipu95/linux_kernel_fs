...
static struct inode *
simplefs_get_inode (struct super_block *sb, const struct inode *dir,
...
static struct inode *
simplefs_iget (struct super_block *sb, unsigned long ino)
{
  struct inode *inode;
  ...
  if (ino == SIMPLEFS_ROOT_INO)
    {
      inode = simplefs_get_inode (sb, NULL, S_IFDIR | 0755, SIMPLEFS_ROOT_INO);
      if (!inode)
...
simplefs_lookup (struct inode *dir, struct dentry *dentry, unsigned int flags)
{
  ...
  for (i = 0; i < sbi->file_count; i++)
    {
      ...
      if (err)
        continue;
      if (!strncmp (meta.name, dentry->d_name.name, dentry->d_name.len)
...
static int
simplefs_readdir (struct file *file, struct dir_context *ctx)
{
  ...
  for (i = 0; i < sbi->file_count; i++)
    {
      ...
      if (!dir_emit (ctx, meta.name,
                     strnlen (meta.name, sbi->max_filename_len), i + 2,
                     DT_REG))
        break;
      ctx->pos = i + 3;
    }
...
int
simplefs_getattr (struct mnt_idmap *idmap, const struct path *path,
  ...
  if (S_ISREG (inode->i_mode) && inode->i_ino >= 2)
    {
      ...
      if (!simplefs_read_file_meta (inode->i_sb, inode->i_ino - 2, &meta))
        i_size_write (inode,
...
int
simplefs_setattr (struct mnt_idmap *idmap, struct dentry *dentry,
                  struct iattr *attr)
{
  ...
  if (attr->ia_valid & ATTR_SIZE)
    {
      ...
      if (!attr->ia_size)
        ...
        meta.data_crc = cpu_to_le32 (
            simplefs_data_crc (sb, index, sbi->max_filename_len));
...
int
simplefs_fill_super (struct super_block *sb, void *data, int silent)
{
...
void
simplefs_kill_sb (struct super_block *sb)
{
  kill_block_super (sb);
}
