...
static void
simplefs_put_super (struct super_block *sb)
{
...
simplefs_mount (struct file_system_type *fs_type, int flags,
...
simplefs_init (void)
{
  ...
  bdev_file = bdev_file_open_by_path (
      disk_name, BLK_OPEN_READ | BLK_OPEN_WRITE, NULL, NULL);
  if (IS_ERR (bdev_file))
    {
