...
static int open_any_file(const char *mnt, int *out_fd, char *path, size_t plen)
{
	DIR *d = opendir(mnt);
	struct dirent *e;
...
static int cmd_wipe(const char *mnt)
{
	char path[PATH_MAX];
	int fd, err = 0;
...
static int cmd_map(const char *mnt, const char *name)
{
	char path[PATH_MAX];
	int fd, i, err = 0;
...
static int run_demo(const char *mnt)
{
	DIR *d = opendir(mnt);
	struct dirent *e;
	int failures = 0;
